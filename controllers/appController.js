...
import ActivityLogModel from "../model/ActivityLog.js";
...
/** GET: http://localhost:8080/api/verify2FAOTP */
export async function verify2FAOTP(req, res) {
  const { code } = req.query;
  if (parseInt(req.app.locals.OTP) === parseInt(code)) {
    req.app.locals.OTP = null; // reset the OTP value
    req.app.locals.resetSession = true; // start session for reset password
...
// add Activiy
/** POST: http://localhost:8080/api/addActivity */
export async function addActivity(req, res) {
  try {
    ...
    if (userId) {
      ...
      // add Activity
      ActivityLogModel.create(body, function (err, data) {
        if (err) throw err;

        return res.status(201).send({ msg: "Activity Added...!" });
      });
    } else {
      return res.status(401).send({ error: "User Not Found...!" });
    }
...
export async function getUsersConversation(req, res) {
  try {
